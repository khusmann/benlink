                            ...
                            OI_BOOL allowPartial)
{
    ...
    if ((*pcmBytes < (sizeof(OI_INT16) * frameSamples * context->common.pcmStride)) && !allowPartial) {
         ...
         * codec frame */
        //printf("-OI_CODEC_SBC_Decode: OI_CODEC_SBC_NOT_ENOUGH_AUDIO_DATA\n");
    ...
    if (context->bufferedBlocks == 0) {
        //printf("Reading scalefactors\n");
        OI_SBC_ReadScalefactors(&context->common, bodyData, &bs);

        //printf("Computing bit allocation\n");
                                     ...
                                     OI_UINT32 *pcmBytes)
{
    ...
    if (context->bufferedBlocks == 0) {
        ...
        if (*frameBytes < bodyLen) {
            //printf("-OI_CODEC_SBC_Decode: OI_CODEC_SBC_NOT_ENOUGH_BODY_DATA\n");
    ...
    //printf("-OI_CODEC_SBC_DecodeRaw: %d\n", status);
    return status;
}
                                   ...
                                   OI_UINT32 *pcmBytes)
{
    ...
    //printf("+OI_CODEC_SBC_DecodeFrame\n");

    //printf("Finding syncword\n");
    ...
    if (*frameBytes < SBC_HEADER_LEN) {
        //printf("-OI_CODEC_SBC_DecodeFrame: OI_CODEC_SBC_NOT_ENOUGH_HEADER_DATA\n");
        return OI_CODEC_SBC_NOT_ENOUGH_HEADER_DATA;
    }
    ...
    if (context->limitFrameFormat && (context->common.frameInfo.subbands != context->restrictSubbands)) {
        //printf("SBC parameters incompatible with loaded overlay\n");
        return OI_STATUS_INVALID_PARAMETERS;
    }
    //printf("Frame: \n");

    if (context->common.frameInfo.nrof_channels > context->common.maxChannels) {
        //printf("SBC parameters incompatible with number of channels specified during reset\n");
        return OI_STATUS_INVALID_PARAMETERS;
    }

    if ((context->common.pcmStride < 1) || (context->common.pcmStride > 2)) {
        //printf("CM stride not set correctly during reset\n");
        return OI_STATUS_INVALID_PARAMETERS;
    }
    ...
    //    //printf("-OI_CODEC_SBC_DecodeFrame: OI_CODEC_SBC_NOT_ENOUGH_BODY_DATA %d < %d\n", *frameBytes, framelen);
    //    return OI_CODEC_SBC_NOT_ENOUGH_BODY_DATA;
    //}
    //printf("frame len %d\n", framelen);
    ...
    //    //printf("CRC Mismatch:  calc=%02x read=%02x\n", crc, context->common.frameInfo.crc);
    //    //printf("-OI_CODEC_SBC_DecodeFrame: OI_CODEC_SBC_CHECKSUM_MISMATCH\n");
     ...
     */
    status = DecodeBody(context, *frameData + SBC_HEADER_LEN, pcmData, pcmBytes, TRUE);
    if (OI_SUCCESS(status)) {
        *frameData += framelen;
        *frameBytes -= framelen;
    }
    //printf("-OI_CODEC_SBC_DecodeFrame: %d\n", status);
